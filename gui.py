...
class Application(tk.Frame):
    def __init__(self, master=None):
        ...
        self.direction_offset = {'Left': (-1, 0), 'Right': (1, 0), 'Up': (0, -1),
                                 'Down': (0, 1)}  # (x,y) = (column,row)
    ...
    def default_frame(self):
        start_width = 50
        start_label = tk.Label(self.frame, text="\n *** COVID Model GUI Display ***\n", width=start_width)
        start_label.grid(row=0, column=0)

        start_label2 = tk.Label(self.frame, text="Start Simulation: File -> Open\n", width=start_width)
        start_label2.grid(row=1, column=0)

        start_label3 = tk.Label(self.frame, text="To reset current model: press the 'r' key \n", width=start_width)
